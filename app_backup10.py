...
c.execute('''CREATE TABLE IF NOT EXISTS esg_data
             (id INTEGER PRIMARY KEY AUTOINCREMENT, empresa TEXT, emisiones_co2 REAL, uso_agua REAL, uso_fertilizantes REAL, impacto_social REAL, score_esg_actual REAL, score_predicho REAL)''')
conn.commit()

# Cargar datos desde SQLite
df = pd.read_sql_query("SELECT empresa, emisiones_co2, uso_agua, uso_fertilizantes, impacto_social, score_esg_actual, score_predicho FROM esg_data", conn)
...
df_dashboard = df.dropna(subset=['score_esg_actual', 'score_predicho'])  # Filtrar para el gráfico de barras
if not df_dashboard.empty:
    fig = px.bar(df_dashboard, x='empresa', y=['score_esg_actual', 'score_predicho'], barmode='group')
    st.plotly_chart(fig)
else:
...
df_emisiones = df[df['emisiones_co2'].notnull()]  # Usar DataFrame sin filtrar score_predicho
if not df_emisiones.empty:
    fig, ax = plt.subplots()
    ax.plot(df_emisiones['empresa'], df_emisiones['emisiones_co2'], marker='o')
...
if st.button("Exportar a Excel"):
    c.execute("SELECT empresa, emisiones_co2, uso_agua, uso_fertilizantes, impacto_social, score_esg_actual, score_predicho FROM esg_data")
    data = c.fetchall()
